...
const Schedules: React.FC = () => {
  ...
  const openEditModal = (s: ScheduleTask) => {
    setEditing(s);
    setModalOpen(true);
  };
  ...
  const handleTrigger = async (s: ScheduleTask) => {
    try {
      ...
      message.success(
        res.taskId ? `已触发，创建任务 #${res.taskId}` : '已触发',
      );
      if (res.taskId) {
    ...
    },
    {
      ...
      render: (_: unknown, r: ScheduleTask) => (
        ...
        >
          <Badge
            status={
              r.totalFailed > 0 ? 'error' : r.totalSuccess > 0 ? 'success' : 'default'
            }
        ...
        <Space size={4}>
          ...
          <Tooltip title="触发历史">
            <Button
              ...
              onClick={() => navigate(`/schedules/${r.id}`)}
            />
          </Tooltip>
          <Tooltip title="编辑">
            <Button
