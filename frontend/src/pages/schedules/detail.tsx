...
const ScheduleDetail: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const hasRunning = triggeredTasks.some((t) =>
      ['PENDING', 'PULLING_CODE', 'PARSING_CODE', 'SPLITTING_TASK',
       ...
       'GENERATING_DOC', 'PUSHING'].includes(t.status),
    );
  ...
  const handleTrigger = async () => {
    try {
      ...
      loadTriggeredTasks();
      if (res.taskId) {
        ModalNavigateToTask(navigate, res.taskId);
      }
    } catch (err) {
      // ignore
    }
        ...
        <Row gutter={16} align="middle">
          <Col flex="auto">
            ...
            <div style={{ color: '#888', marginTop: 6 }}>
              ID #{schedule.id} · 创建于 {dayjs(schedule.createdAt).format('YYYY-MM-DD HH:mm:ss')}
        ...
        <Table
          ...
          columns={[
            ...
            },
            {
              ...
              width: 110,
              render: (v: string) => (
                <Tag color={fireStatusColor[v] ?? 'blue'}>{v}</Tag>
              ),
            },
            {
              title: '关联任务',
...
function ModalNavigateToTask(navigate: (path: string) => void, taskId: number) {
  setTimeout(() => {
